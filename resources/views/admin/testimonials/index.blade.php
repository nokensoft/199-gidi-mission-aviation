            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4">
                        ...
                        <button class="text-xs px-2 py-1 rounded-full font-medium cursor-pointer {{ $t->is_approved ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600' }}">{{ $t->is_approved ? 'Disetujui' : 'Pending' }}</button></form>
                    </td>
                    <td class="px-6 py-4">
                        <form method="POST" action="{{ route('admin.testimonials.toggle-featured', $t) }}" class="inline">@csrf
                        <button class="text-xs px-2 py-1 rounded-full font-medium cursor-pointer {{ $t->is_featured ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-400' }}">{{ $t->is_featured ? 'Ya' : 'Tidak' }}</button></form>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-1.5">
                            <a href="{{ route('admin.testimonials.edit', $t) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition" title="Edit"><i class="fa-solid fa-edit text-xs"></i></a>
                            <form method="POST" action="{{ route('admin.testimonials.destroy', $t) }}" class="inline" onsubmit="event.preventDefault(); confirmDelete(this, 'Hapus Testimoni', 'Apakah Anda yakin ingin menghapus testimoni dari &quot;{{ addslashes($t->name) }}&quot;?')">@csrf @method('DELETE')<button class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 transition cursor-pointer" title="Hapus"><i class="fa-solid fa-trash text-xs"></i></button></form>
                        </div>
                    </td>
                </tr>
                @empty
